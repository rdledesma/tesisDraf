...
\begin{enumerate}
    ...
    Además, dentro de este grupo se encuentran las denominadas \textit{Adaptaciones regionales}, que emplean estaciones cercanas. Estos métodos utilizan un período de superposición de datos derivados de terreno y modelo para desarrollar una corrección aplicable a todo el registro del modelo, incluso en períodos sin datos superpuestos. Por lo tanto, su aplicación requiere disponer de al menos 9--12 meses de datos simultáneos para capturar efectos estacionales. Sin embargo, la disminución de la autocorrelación temporal al aumentar el tiempo entre los períodos de entrenamiento y corrección del modelo suele no considerarse, lo que puede resultar en sobrecorrecciones o subcorrecciones. Este aspecto es especialmente crítico para el DNI, cuya variabilidad interanual es mayor que la del GHI \citep{Lohmann2006}.\\
    ...
    \item \textbf{Combinación de datos satelitales con modelos meteorológicos numéricos (NWP)}\\
...
Las métricas más relevantes en SA incluyen \textbf{MBE}, \textbf{MAE} y \textbf{RMSE}, así como sus versiones normalizadas \textbf{rMBE}, \textbf{rMAE} y \textbf{rRMSE}, que permiten evaluar tanto el sesgo como la dispersión del error.

El MAE cuantifica el error medio absoluto entre valores observados y estimados. El RMSE penaliza en mayor medida los errores grandes, siendo sensible a discrepancias puntuales. El rMBE expresa el sesgo relativo, indicando si el modelo sobreestima o subestima de forma sistemática. En conjunto, estas métricas permiten evaluar la fidelidad del ajuste y posibles desviaciones sistemáticas.



\section{Síntetis del Capítulo}
