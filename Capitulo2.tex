...
Aunque cada definición destaca aspectos particulares del proceso, todas convergen en un propósito fundamental: ajustar los datos satelitales o modelados para que reflejen con mayor precisión las condiciones reales de irradiancia en un punto geográfico específico. Dicho de otro modo, la Adaptación al Sitio consiste en aplicar técnicas estadísticas que permitan reducir tanto el sesgo como la dispersión inherentes a los productos derivados de satélites, utilizando mediciones terrestres como patrón de referencia.


\subsubsection{Proceso conceptual}
...
Si bien la definición original de Samuel no lo menciona directamente, un aspecto fundamental del aprendizaje automático es el \textit{autoaprendizaje}. Este concepto implica el uso de modelos estadísticos para identificar patrones y optimizar el rendimiento con base en datos e información empírica, sin requerir instrucciones de programación explícitas \cite{Theobald2024}.
...
\item {Regresión}: error cuadrático medio (MSE), error absoluto medio (MAE), $R^2$.
\end{itemize}
...
Las métricas más relevantes en SA incluyen MAE, RMSE, rMBE y rRMSD, que permiten evaluar tanto el sesgo como la dispersión del error.
