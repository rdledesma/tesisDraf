...
donde N es el número de árboles y $\hat{y}_i$ corresponde a la predicción del árbol i. En el caso de clasificación, se utiliza votación mayoritaria.
...
El algoritmo \textit{Extreme Gradient Boosting} (XGBoost) es una implementación optimizada del método de \textit{Gradient Boosted Decision Trees} (GBDT) \citep{chen2016xgboost}.
...
XGBoost se ha consolidado como estado del arte en aprendizaje automático debido a su alto desempeño en tareas de clasificación y regresión en diversos dominios \citep{espinoza2020aplicacion}.\\
...
\foreach \i in {1,2,3,4}{ \draw[flecha] (exp\i.east) -- +(2,0) |- (final.west); } \end{tikzpicture} \caption[Analogía del comité de expertos en XGBoost.]{Analogía del comité de expertos en XGBoost: cada árbol corrige los errores de sus predecesores y contribuye a una predicción final más precisa de la GHI.} \label{fig:xgb_experts} \end{figure}
...
\item \textbf{Validación}: empleado durante el entrenamiento para seleccionar hiperparámetros y evitar el sobreajuste (\textit{overfitting}). Una proporción habitual es 80\% para entrenamiento y 20\% para validación dentro del subconjunto inicial de entrenamiento.
\item \textbf{Prueba (test)}: reservado exclusivamente para la evaluación final. Nunca se utiliza en la fase de ajuste, ya que su propósito es ofrecer una estimación imparcial del error de generalización.
\end{itemize}
...
Las métricas más relevantes en SA incluyen \textbf{MBE} (Error Medio de Sesgo), 
...
El MAE cuantifica el error medio absoluto entre valores observados y estimados, 
