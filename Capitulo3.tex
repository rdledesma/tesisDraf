...
Heliosat-4 (26) es un modelo completamente físico que emplea un enfoque rápido pero preciso para el modelo de transferencia radiativa libRadtran (27). Este modelo utiliza diversos datos satelitales y la base de datos de reanálisis del Servicio de Monitoreo Atmosférico de Copernicus (CAMS). Las propiedades de las nubes se derivan de imágenes satelitales de Meteosat de Segunda Generación (MSG) con una frecuencia de 15 minutos, utilizando un esquema APOLLO adaptado (Esquema de Procesamiento AVHRR3 sobre Nubes, Tierra y Océano) (28). La versión operativa de Heliosat-4 se presenta en forma de tablas de consulta, lo que permite un cálculo rápido. Este modelo ya se ha evaluado en la región en estudios como. Este conjunto de datos se descargó del sitio web de SoDa con una resolución temporal de 15 minutos.El tamaño de píxel del MSG en la región es de aproximadamente 16 km. Es necesario indicar que existe también una alternativa que permite descargar los estimativos directamente desde un servicio de transferencia de archivos en link de CAMS-Volumen.\\


El producto Flujos de Radiación de Onda Corta en Superficie Descendente MSG - Total y Difusa (MDSSFTD) es una estimación instantánea (cada 15 minutos) del flujo de radiación de onda corta descendente global y difusa en la superficie. El método para obtener este producto consta de dos módulos separados: uno para cielo despejado y otro para cielo nublado (35),(36). El producto de cielo despejado utiliza el enfoque descrito en la referencia (37). Este método, conocido como SIRAMix (Radiación Incidente en Superficie usando Mezclas de Aerosoles), es un método de parametrización física acoplado a una tabla de consulta (LUT) precalculada de las propiedades de los aerosoles, que consiste principalmente en las transmitancias directa y difusa de los diferentes componentes de los aerosoles y sus correspondientes albedos. La LUT se genera mediante modelos de transferencia radiativa para variar la carga de aerosoles, el vapor de agua y el ángulo cenital solar. El método de recuperación del GHI en condiciones nubosas utiliza el albedo total de las nubes de onda corta en la parte superior de la atmósfera (TOA), obtenido a partir de las reflectancias TOA observadas del MSG. Utiliza las mismas estimaciones de aerosoles que en el caso de cielo despejado, y los términos relacionados con las nubes se determinan mediante un modelo simplificado de transferencia radiativa, similar al método de la referencia (38). Finalmente, estas transmitancias, a saber, la transmitancia efectiva en cielo despejado estimada mediante gases y aerosoles, y la transmitancia de las nubes estimada mediante el modelo simplificado de transferencia radiativa, se utilizan conjuntamente para determinar el GHI. Este modelo tiene una resolución espacial aproximada de 3 km con una frecuencia de actualización de 15 minutos, y sus estimaciones pueden descargarse en formato NetCDF desde https://datalsasaf.lsasvcs.ipma.pt/PRODUCTS/MSG/MDSSFTD.\\


ERA5 es el modelo de reanálisis atmosférico de quinta generación desarrollado por el Centro Europeo de Pronósticos Meteorológicos a Plazo Medio (CEPMPM), que abarca el período comprendido entre enero de 1940 y la actualidad. Este modelo proporciona estimaciones horarias de numerosas variables climáticas, incluyendo aspectos atmosféricos, terrestres y oceánicos. Los datos se presentan en una cuadrícula horizontal de 30 km y la atmósfera se clasifica utilizando 137 niveles que van desde la superficie de la Tierra hasta una altitud deTABLA IIIRESUMEN DE MODELOS UTILIZADOSModelo Disponibilidad Cobertura temporal Área de resolución (mín.)Heliosat-4 2004 1, 15, 30, 60 -66° a 66° hasta la actualidad en latitud y longitudNSRDB 2019 10, 30, 60 toda la regiónhasta 2023DSR 2020 60 toda la regiónhasta la actualidadLSA-SAF 2004 15 toda la regiónhasta la actualidadMERRA-2 1980 60 toda la regiónhasta la actualidadERA-5 1940 60 toda la regiónhasta la actualidadImágenes GOES-16 2017 15, 10 desde 2020 toda la regiónhasta la actualidad80 km [47]. El modelo se basa en el Sistema Integrado de Pronóstico (IFS) Cy41r2, operativo hasta 2016. Por lo tanto, ERA5 se beneficia de una década de avances en física de modelos, dinámica de núcleos y asimilación de datos, en comparación con el conjunto de datos ERA-Interim anterior. Además de una resolución horizontal significativamente mejorada de 0,25° de latitud (31 km), ERA5 proporciona resultados horarios. Este modelo combina observaciones satelitales e in situ en sus estimaciones mediante técnicas avanzadas de modelado y asimilación de datos; por lo tanto, se ha demostrado que ERA5 produce simulaciones más precisas en comparación con generaciones anteriores [48], [49]. Los datos están disponibles gratuitamente en https://cds.climate.copernicus.eu/datasets/reanalysis-era5-single-levels?tab=download.\\


El Análisis Retrospectivo de la Era Moderna para Investigación y Aplicaciones, versión 2 (MERRA-2) [42] es un conjunto de datos de reanálisis desarrollado por la NASA con el sistema de asimilación de datos atmosféricos Goddard Earth Observing System (GEOS-5.12.4) [43], [44]. Se utiliza para estimar diversos parámetros atmosféricos, incluyendo la irradiancia de onda corta descendente (GHI). El conjunto de datos abarca el período comprendido entre 1980 y la actualidad con una resolución espacial de latitud-longitud de 0,5 × 0,625° y un intervalo de tiempo de 60 minutos. Este modelo mejora el modelado de aerosoles e información relacionada que afecta a la irradiancia solar en cielo despejado, en comparación con la versión 1 de MERRA [45]. También incluye otras mejoras físicas, como el contenido de vapor de agua y el modelado de nubes [46], que afectan a las estimaciones de la irradiancia solar en todas las condiciones del cielo. Como resultado de estas actualizaciones, se espera que el conjunto de datos proporcione un mejor rendimiento de la irradiancia solar que su predecesor. Los datos están disponibles gratuitamente en https://disc.gsfc.nasa.gov/datasets?project=MERRA-2.\\


Adicionalmente, existen otros modelos de estimación que cubren nuestro territorio de estudio, como el modelo PSM de la NSRDB, cuyo desempeño se ha presentado como el más prometedor según las evaluaciones de (citar)(citar). Sin embargo, debido a su limitada cobertura temporal, hemos decidido omitir su consideración en este trabajo.\\
...
Los resultados correspondientes se presentan en la Tabla \ref{tab:metrics-as-5}, donde puede observarse una mejora general en las métricas, lo que evidencia el beneficio de incorporar información térmica como variable auxiliar en el proceso de adaptación.
...
\begin{figure}
    ...
    \caption{RMSD(\%) comparativo para las evaluaciones utilizando el enfoque de celdas adyacentes}
...
El análisis presentado en las Secciones \ref{sec:01} y \ref{sec:02} permitió establecer que, en el contexto de la adaptación al sitio (AS), el grado de mejora obtenido en las estimaciones de la irradiancia global horizontal (GHI) no depende necesariamente de la complejidad de los modelos regresores utilizados. Más bien, los resultados evidencian la importancia de una adecuada selección de variables regresoras para garantizar un desempeño correcto y estable de los modelos de regresión.
...
\caption{Matriz de RMSD (\%) obtenida a partir de la validación cruzada entre sitios para el modelo ERA5, utilizando los modelos ajustados con celdas satelitales adyacentes al sitio de interés.}
