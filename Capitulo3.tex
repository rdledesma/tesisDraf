...
En las secciones anteriores hemos evaluado distintos enfoques para la adaptación al sitio, evidenciando el potencial de este proceso dependiendo de las características del modelo de estimación y del sitio donde se aplique dicho proceso. Sin embargo, cada uno de los enfoques implementados fue testado en el mismo sitio en el que se ajustó; esto corresponde al proceso tradicional de adaptación al sitio. Resulta de gran interés analizar el comportamiento de los modelos de regresión cuando se aplican en sitios distintos de aquellos en los que fueron entrenados. A este enfoque lo denominamos \textbf{validación cruzada entre sitios}.
...
La validación cruzada entre sitios permite evaluar si los ajustes aplicados en un sitio pueden extenderse a un territorio más amplio. Esto resulta especialmente útil para el desarrollo de modelos regionales o mapas climáticos basados en técnicas de adaptación al sitio, donde se busca reducir la dependencia de calibraciones locales y aprovechar patrones espaciales comunes.
...
La Figura \ref{fig:crossValidation} muestra la matriz de sensibilidad del RMSD (\%) obtenida a partir de las validaciones cruzadas realizadas para cada sitio usando los modelos MLR ajustados (cuyos resultados fueron presentados en la Tabla \ref{tab:metrics-final}). Cada celda representa el desempeño del modelo entrenado en un sitio (filas) y aplicado en otro (columnas), permitiendo identificar patrones de transferencia entre sitios.

En general, se observa que el error aumenta cuando los modelos se aplican en sitios con características ambientales marcadamente distintas a las del sitio de entrenamiento. Esto indica que la capacidad de extrapolación espacial de los modelos de adaptación al sitio es limitada y depende fuertemente de la similitud climática y topográfica entre las estaciones.
...
Cada fila representa el sitio de entrenamiento y cada columna el sitio de prueba. Los valores positivos indican que la adaptación al sitio reduce el error respecto al modelo ERA5 sin ajustar, mientras que los valores negativos muestran un empeoramiento.

\textbf{(1)} En primer lugar, se aprecia que los modelos entrenados en sitios de baja altitud (como YU) tienden a presentar un mejor desempeño cuando se aplican en sitios con condiciones altitudinales similares, mientras que su transferencia a zonas de mayor altitud resulta menos efectiva.

\textbf{(2)} En contraste, los modelos ajustados en sitios de alta montaña (por ejemplo, LQ y ERO) muestran cierta robustez cuando se aplican en otros sitios de altura comparable. Esto sugiere que la variable altitud, al influir fuertemente en la estructura térmica y la humedad del aire, actúa como un factor limitante en la generalización de los modelos.

\textbf{(3)} Los valores negativos en la matriz $\Delta \text{rRMSD}$ reflejan los casos donde la adaptación al sitio no solo no mejora el desempeño, sino que incluso introduce sesgos. Este comportamiento podría deberse a sobreajustes locales o a la ausencia de representatividad espacial de las variables predictoras en otros contextos geográficos.

\textbf{(4)} Asimismo, puede observarse que algunos sitios actúan como “donantes” más universales, es decir, presentan modelos que se transfieren con mejor desempeño general. Identificar estos sitios resulta clave para optimizar la estrategia de calibración regional y reducir la necesidad de entrenamientos específicos en cada punto.

\textbf{(5)} Por otra parte, la magnitud de las mejoras observadas (valores positivos en $\Delta \text{rRMSD}$) tiende a ser mayor cuando los sitios comparten condiciones climáticas semejantes, lo que refuerza la idea de que la coherencia espacial de las variables predictoras es esencial para la eficacia de la adaptación al sitio.

\textbf{(6)} Este análisis también permite evidenciar la direccionalidad de la transferencia: un modelo entrenado en un sitio A puede funcionar bien en un sitio B, pero no necesariamente a la inversa. Ello sugiere una dependencia asimétrica asociada a las características de variabilidad local y regional.

\textbf{(7)} En términos prácticos, la validación cruzada entre sitios constituye una herramienta valiosa para definir el grado de representatividad espacial de los modelos. Permite identificar qué ajustes locales pueden ser reutilizados en áreas vecinas y cuáles requieren recalibración específica.

\textbf{(8)} Finalmente, los resultados aquí presentados destacan la importancia de integrar información geográfica adicional (como gradientes de elevación, cobertura del suelo o proximidad al mar) para mejorar la capacidad de generalización de los modelos de adaptación al sitio. De este modo, se avanzaría hacia la construcción de modelos híbridos que combinen la precisión local con la coherencia regional.
